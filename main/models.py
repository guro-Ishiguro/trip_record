...
class Reservation(models.Model):
    ...
    trip = models.ForeignKey(
        Trip, on_delete=models.CASCADE, related_name="reservations"
    )
    reservation_type = models.CharField(
        max_length=3,
        choices=ReservationType.choices,
        default=ReservationType.OTHER,
        verbose_name="予約の種類",
    )
    ...
    confirmation_number = models.CharField(
        max_length=100, blank=True, verbose_name="予約番号"
    )
    amount = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="金額"
    )
...
class Expense(models.Model):
    ...
    category = models.CharField(
        max_length=5, choices=ExpenseCategory.choices, verbose_name="カテゴリ"
    )
    ...
    payment_method = models.CharField(
        max_length=50, blank=True, verbose_name="支払い方法"
    )
...
class TodoCategory(models.Model):
    trip = models.ForeignKey(
        Trip, on_delete=models.CASCADE, related_name="todo_categories"
    )
...
class TodoItem(models.Model):
    category = models.ForeignKey(
        TodoCategory, on_delete=models.CASCADE, related_name="items"
    )
